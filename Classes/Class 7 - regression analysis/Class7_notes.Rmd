...
read_delim() comes from tidyverse and needs us to specify the 'delim' argument, i.e. what character separates values. Normally it's either white space, tab, comma or semicolon. When you try to read files, you can try experimenting with it. In this case it's the space character:
...
plot1 <- ggplot(child, aes(Television, Aggression)) + 
  geom_point() + 
  geom_smooth(method = lm, se = F) + #fit the best straight line to the data
  ggtitle('Scatterplot with regression')

plot2 <- ggplot(child, aes(Computer_Games, Aggression)) + 
  geom_point() + 
  geom_smooth(method = lm, se = F) + #fit the best straight line to the data
  ggtitle('Scatterplot with regression')

plot3 <- ggplot(child, aes(Sibling_Aggression, Aggression)) + 
  geom_point() + 
  geom_smooth(method = lm, se = F) + #fit the best straight line to the data
  ggtitle('Scatterplot with regression')

plot4 <- ggplot(child, aes(Diet, Aggression)) + 
  geom_point() + 
  geom_smooth(method = lm, se = F) + #fit the best straight line to the data
  ggtitle('Scatterplot with regression')

plot5 <- ggplot(child, aes(Parenting_Style, Aggression)) + 
  geom_point() + 
  geom_smooth(method = lm, se = F) + #fit the best straight line to the data
  ggtitle('Scatterplot with regression')
