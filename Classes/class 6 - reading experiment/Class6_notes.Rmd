...
data <- lapply(files, read.csv) %>%  # apply read_csv() to every element in files list and send the resulting tibbles to the next line
  rbind.fill()  #bind rows from resulting tibbles together so you have one dataset with all of your data

data$RT <- ifelse(is.na(data$Reaction_Time), data$Reaction.time, data$Reaction_Time) %>% select(-c(data$Reaction_Time, data$Reaction.time))
