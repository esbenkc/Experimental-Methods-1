...
```{r}
df %>% group_by(taste_cola) %>% summarise(mean_music = mean(hours_music_per_week))
...
```{r}
e_boxplot <- function(x){
  ggplot(x, aes(x = taste_cola, y = hours_music_per_week, color = taste_cola)) +
  geom_boxplot() +
...
e_boxplot(df)
```
The box plot shows that there's a lot of outliers above the upper whisker which might skew the mean (especially at option S).
...
Yes, there are a lot of data points beyond the upper whisker. Let's remove some of the extreme outliers (>40 hours).

```{r}
in_df <- df[df$hours_music_per_week <= 3 * sd(df$hours_music_per_week),]
e_boxplot(in_df)
in_df %>% group_by(taste_cola) %>% summarise(mean_music = mean(hours_music_per_week))
...
round(stat.desc(df$hours_music_per_week, basic = F, norm = T), digits = 4)
...
ggplot(in_df, aes(x = hours_music_per_week)) + #note that I use 'filtered' dataframe here :) It might be called differently for you
  geom_histogram(aes(y = ..density..), binwidth = 0.25) +
  ggtitle("This is what music time looks like without the biggest outlier") +
  stat_function(fun = dnorm, args = list(mean = mean(in_df$hours_music_per_week, na.rm = TRUE), sd = sd(in_df$hours_music_per_week, na.rm = TRUE)), colour= "darkgreen", size = 1)+
  theme_classic()
...
qqnorm(in_df$hours_music_per_week)
```
...
```{r}
ggplot(df, aes(x = df$choose_rand_num)) +
  geom_histogram()
```
...
```{r}
ggplot(df, aes( sample = df$shoesize )) + qqplot() + qqplot_line()
```
